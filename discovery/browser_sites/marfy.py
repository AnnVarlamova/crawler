...
MARFY_LISTING_READY_SELECTORS = [
    "#js-product-list",
    "#js-product-list .products.row.products-grid",
    "#js-product-list .js-product-miniature-wrapper",
]

MARFY_PRODUCT_LINK_SELECTOR = "#js-product-list .js-product-miniature-wrapper a[href]"
...
async def _wait_for_listing(page: Page) -> None:
    for selector in MARFY_LISTING_READY_SELECTORS:
        try:
            await page.locator(selector).first.wait_for(timeout=15000)
            logger.info("[marfy] listing appeared via selector=%s", selector)
            return
        except Exception:
            continue
...
async def _get_page_links(page: Page) -> list[str]:
    return await collect_unique_links(
        page=page,
        selector=MARFY_PRODUCT_LINK_SELECTOR,
        href_filter=lambda href: isinstance(href, str) and href.startswith("https://www.marfy.it/"),
    )
...
async def run(browser: Browser, spec: dict) -> list[str]:
    ...
    try:
        ...
        runlog.info("START marfy %s", category)
        ...
        for page_num in range(page_from, page_to + 1):
            ...
            logger.info("[marfy] open page=%s url=%s", page_num, page_url)
            ...
            logger.info(
                "[marfy] page=%s found=%s fresh=%s total=%s",
                page_num,
                len(links),
                len(fresh_links),
                len(all_links),
            ...
            if stagnant_rounds >= 3:
                logger.info("[marfy] stop by stagnant pages at page=%s", page_num)
                break

        runlog.info("DONE marfy %s total=%s", category, len(all_links))
        return all_links
