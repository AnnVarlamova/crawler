...
HELPERSEW_PRODUCT_SELECTOR = "a.cat-card__name[href]"


def _build_page_url(base_url: str, page_num: int, needs_pagination: bool) -> str:
    if not needs_pagination or page_num <= 1:
        return base_url
    return f"{base_url}?PAGEN_1={page_num}"


async def _wait_for_listing(page: Page) -> None:
    try:
        await page.locator(HELPERSEW_PRODUCT_SELECTOR).first.wait_for(timeout=15000)
        logger.info("[helpersew] listing appeared")
        return
    except Exception:
...
async def _get_page_links(page: Page) -> list[str]:
    return await collect_unique_links(
        page=page,
        selector=HELPERSEW_PRODUCT_SELECTOR,
        href_filter=lambda href: isinstance(href, str) and "/catalog/" in href and href.count("/") > 4,
    )
...
async def run(browser: Browser, spec: dict) -> list[str]:
    ...
    try:
        ...
        runlog.info("START helpersew %s", category)
        ...
        for page_num in range(page_from, page_to + 1):
            ...
            logger.info("[helpersew] open page=%s url=%s", page_num, page_url)
            ...
            logger.info(
                "[helpersew] page=%s found=%s fresh=%s total=%s",
                page_num,
                len(links),
                len(fresh_links),
                len(all_links),
            ...
            if needs_pagination and stagnant_rounds >= 2:
                logger.info("[helpersew] stop by stagnant pages at page=%s", page_num)
                break

            if not needs_pagination:
                break

        runlog.info("DONE helpersew %s total=%s", category, len(all_links))
        return all_links
